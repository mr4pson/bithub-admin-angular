...
export class CMailingRepository extends CRepository<CMailing> {
    ...
    public loadChunk(part: number = 0, chunkLength: number = 10, sortBy: string = "id", sortDir: number = 1, filter: any = {}): Promise<CChunk<CMailing>> {
        const dto: IGetList = {from: part * chunkLength, q: chunkLength, sortBy, sortDir, filter};
        return new Promise((resolve, reject) =>
            this.dataService
                .mailingsChunk(dto)
                .subscribe({
                    next: res => res.statusCode === 200 ? resolve(new CChunk<CMailing>(res.data.map(d => new CMailing().build(d)), res.elementsQuantity, res.pagesQuantity)) : reject(res.error),
    ...
    public loadOne(id: number): Promise<CMailing> {
        return new Promise((resolve, reject) =>
            this.dataService
                .mailingsOne(id)
                .subscribe({
                    next: res => res.statusCode === 200 ? resolve(new CMailing().build(res.data)) : reject(res.error),
                    error: err => reject(err.message)
                }));
    }
    ...
    public create(x: CMailing): Promise<CMailing> {
        ...
        return new Promise((resolve, reject) =>
            this.dataService
                .mailingsCreate(fd)
                .subscribe({
                    next: res => res.statusCode === 201 ? resolve(new CMailing().build(res.data)) : reject(res.error),
                    error: err => reject(err.message)
                }));
    }
