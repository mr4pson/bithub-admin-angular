...
import { CRepository } from '../services/repositories/_repository';
import { CAppService } from '../services/app.service';
import { CListService } from '../services/list.service';
...
export abstract class CListPage<T extends CEntity> extends CSomePage {
    ...
    get canBulk(): boolean {return !!this.xl.filter(x => x.__selected).length;}
    ...
    public async initList(reloading: boolean = false): Promise<void> {
        try {
            ...
            if (this.part > 0 && !this.chunk.data.length) { // after deleting or updating may be currentPart is out of possible diapason, then decrease and reload again
                this.part--;
