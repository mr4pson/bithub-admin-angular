...
export class CPanelSelectComponent
  ...
  implements OnChanges
{
  ...
  get repository(): CRepository<CTranslatableEntity<any>> {
  ...
  get scrolledToBottom(): boolean {
    return (
      ...
      this.container.clientHeight + 200
    );
  }
  ...
  private async initEntities(): Promise<void> {
    try {
      ...
      this.part = 0;
      const chunk = await this.repository.loadChunk(
        this.part,
        this.chunkLength,
        this.sortBy,
        this.sortDir,
      ...
      this.exhausted =
        !chunk.elementsQuantity || this.part + 1 === chunk.pagesQuantity;
      this.loading = false;
    } catch (err) {
